...
ifeq ($(strip $(NO_SMDH)),)
	export _3DSXFLAGS += --smdh=$(CURDIR)/bin/$(TARGET).smdh
endif
...
clean:
	@echo clean ...
# @rm -fr $(BUILD) bin/$(TARGET).3dsx $(OUTPUT).smdh bin/$(TARGET).elf bin/$(TARGET).cia $(GFXBUILD)
	@rm -fr $(BUILD) $(BIN) $(GFXBUILD)
...
cia:
	...
	@.\tools\makerom.exe -f cia -o bin\LeapSync.cia -target t -i build\LeapSync.cxi:0:0 -i build\LeapSync.cfa:1:1 -major 0 -minor 8 -micro 0
